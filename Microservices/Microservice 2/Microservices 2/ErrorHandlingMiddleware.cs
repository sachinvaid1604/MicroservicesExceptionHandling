...
namespace AEG.Common.Middlewares
{
    ...
    public class ErrorHandlingMiddleware
    {
        ...
        private static async Task HandleInternalServerError(HttpContext context, Exception ex, HttpResponse response, bool isCustomException = false)
        {
            ...
            await response.WriteAsync(JsonConvert.SerializeObject(new ResponseClass() { Content = "This is the content sent by Microservice 2", StatusCode = 222, Message = "Type of original Exception is  " + ex.GetType().ToString() + "  And the Exception Message is :" + ex.Message + " With Stack Trace :" + ex.StackTrace }
            ));
            // throw ex;
        }
        ...
        private static async Task HandleCustomException(HttpContext context, Exception ex, HttpResponse response, bool isCustomException = false)
        {
            ...
            await response.WriteAsync(JsonConvert.SerializeObject(new ResponseClass() { Content = "This is the content sent by Microservice 2 for HandleCustomException", StatusCode = 224, Message = "This is the message with exception stack trace " + ex.StackTrace }
            ));
        }
        ...
        {
            context.Response.StatusCode = (int)HttpStatusCode.BadRequest;
            await response.WriteAsync(JsonConvert.SerializeObject(new ResponseClass() { Content = "This is the content sent by Microservice 2 for DivideByZeroException", StatusCode = 221, Message = "This is the message with exception stack trace " + ex.StackTrace }
           ));
        }
    }
